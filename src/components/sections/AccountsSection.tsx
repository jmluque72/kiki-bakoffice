...
export const AccountsSection: React.FC<AccountsSectionProps> = ({ isReadonly = false }) => {
  ...
  const QUICK_NOTIFICATION_TYPES: { code: string; label: string; description?: string }[] = [
  ...
  const openModal = async (account?: Account) => {
    ...
    if (account) {
      ...
      try {
        ...
        const existingQuick =
          configData.quickNotificationSettings && configData.quickNotificationSettings.length > 0
            ? configData.quickNotificationSettings
            : QUICK_NOTIFICATION_TYPES.map(t => ({
                code: t.code,
                enabled: true
              }));
                ...
                {editingAccount && (
                  <div className="border-t border-gray-200 pt-4 mt-4 space-y-4">
                          ...
                          <label className="flex items-center space-x-3 cursor-pointer">
                            <input
                              ...
                              onChange={(e) => setAccountConfig(prev => ({
                                ...(prev || { quickNotificationSettings: QUICK_NOTIFICATION_TYPES.map(t => ({ code: t.code, enabled: true })) }),
                    ...
                    <div className="pt-2 border-t border-gray-200">
                      <h4 className="text-sm font-semibold text-gray-900 mb-3">Comunicaciones rápidas habilitadas</h4>
                      {loadingConfig ? (
                        <div className="flex items-center justify-center py-4">
                          <Loader2 className="w-5 h-5 animate-spin text-blue-600" />
                          <span className="ml-2 text-sm text-gray-600">Cargando configuración...</span>
                        ...
                        <div className="space-y-2">
                          <p className="text-xs text-gray-500">
                            Selecciona qué tipos de comunicaciones rápidas pueden usar esta institución en la app.
                          </p>
                            ...
                            <table className="min-w-full text-sm">
                              ...
                              <tbody>
                                {QUICK_NOTIFICATION_TYPES.map(type => {
                                  const enabled =
                                    accountConfig?.quickNotificationSettings?.find(s => s.code === type.code)?.enabled ?? true;
                                  return (
                                          ...
                                          <input
                                            ...
                                            onChange={(e) =>
                                              setAccountConfig(prev => {
                                                ...
                                                const current = base.quickNotificationSettings || [];
